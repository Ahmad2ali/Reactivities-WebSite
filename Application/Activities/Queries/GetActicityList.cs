        ...
        public async Task<Result<PagedList<ActivityDto, DateTime?>>> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            if (!string.IsNullOrEmpty(request.Params.Filter))
            {
                query = request.Params.Filter switch
                {
                    ...
                    "isHost" => query.Where(x => x.Attendees.Any(a => a.UserId == userAccessor.GetUserId())),
                    _ => query
                };
            }

            var progectedActicities = query.ProjectTo<ActivityDto>(mapper.ConfigurationProvider, new { currentUserId = userAccessor.GetUserId() });

            var activities = await progectedActicities
