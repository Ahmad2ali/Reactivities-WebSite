        ...
        <Box sx={{ display: "flex", gap: 2 }}>
          {activity.isHost ? (
            <>
              <StyledButton
                variant="contained"
                color={activity.isCncelled ? "success" : "error"}
                onClick={() => updateAttendance.mutate(activity.id)}
                disabled={updateAttendance.isPending}
              >
                {activity.isCncelled ? "Re-activate Activity" : "Cancel Activity"}
              </StyledButton>
              <StyledButton
                variant="contained"
                color="primary"
            ...
            <StyledButton
              ...
              onClick={() => {updateAttendance.mutate(activity.id)}}
              disabled={updateAttendance.isPending || activity.isCncelled}
            >
